  ...
  return (
    <>
      <NavbarParent>
        <NavbarWrapper>
          <div className="left">
            ...
            <div className="links">
              {leftLinks.map((v, i) => (
                <TopLevelLink id={`nav_toplevel_left-link-${i}`} {...v} />
              ))}
            </div>
          </div>
          ...
          <div className="right">
            <div className="links">
              {rightLinks.map((v, i) => (
                <TopLevelLink id={`nav_toplevel_right-link-${i}`} {...v} />
              ))}
            </div>
            <div
              className="ctas"
      ...
      <MobileNavbarWrapper className={isOpen ? 'opened' : 'closed'}>
        ...
        >
          {links.map((v, i) => (
            <TopLevelLink mobile id={`nav_toplevel_left-link-${i}`} {...v} />
          ))}
        </div>
        ...
        >
          {ctas.map((v, i) => (
            <Button id={`cta-mobile-${i}`} css={{ width: '100%' }} {...v} />
          ))}
        </div>
      </MobileNavbarWrapper>
    </>
